...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function showPostDetails(post) {
    ...
    window.location.href = `post.html?postData=${encodeURIComponent(postDataJson)}`;
  }
  ...
  function createPostCard(post) {
    ...
    card.innerHTML = `
      ...
      <div class="post-content">
        ...
        <p><span class="ellipse-icon" data-post-id="${post.id}" title="Read More">&#8230;</span></p>
        
      </div>
    `;

    // Get a reference to the ellipse-icon element
    const ellipseIcon = card.querySelector('.ellipse-icon');
    ...
    ellipseIcon.addEventListener('click', function () {
  ...
  function deletePost(id) {
    const postsDataIndex = postsData.findIndex((post) => post.id === id);
    if (postsDataIndex > -1) {
      postsData.splice(postsDataIndex, 1);
    }
  ...
  deleteButtons.forEach((button) => {
    button.addEventListener('click', function () {
      const postId = parseInt(this.closest('.post-card').getAttribute('data-post-id'));
      console.log(postId);
      showModal(postId);
    });
  });
